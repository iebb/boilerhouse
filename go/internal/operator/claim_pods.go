...
func (r *ClaimReconciler) findTenantPod(ctx context.Context, ns, tenantId, workloadRef string) (*corev1.Pod, error) {
	...
	// recreated under the SAME name (a cordless reset) must not latch onto the
	// previous incarnation's dying pod: handleNewClaim would wait on it forever
	// (it never becomes Running) so the box never cold-boots — the reset phantom
	// (claim Active, no pod). Treat a terminating pod as absent → cold boot.
